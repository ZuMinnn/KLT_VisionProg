...
def getFeatures(img,bbox,use_shi=False):
    ...
    for i in range(n_object):
        ...
        xmax = min(xmin + boxw, w)
        ymax = min(ymin + boxh, h)
        boxw = xmax - xmin
        boxh = ymax - ymin
        
        # Check if ROI is valid (at least 10x10 pixels)
        if boxw < 10 or boxh < 10:
            print(f'Warning: Bounding box too small ({boxw}x{boxh}), skipping object {i}')
            temp[i] = np.empty((0, 2))
            continue
            
        roi = img[ymin:ymax,xmin:xmax]
        
        # Double check ROI is not empty
        if roi.size == 0:
            print(f'Warning: Empty ROI for object {i}, skipping')
            temp[i] = np.empty((0, 2))
            continue
