...
def objectTracking(rawVideo, draw_bb=False, play_realtime=False, save_to_file=False):
    ...
    for i in range(1,n_frame):
        ...
        if n_features_left < 15:
            ...
            new_startXs, new_startYs = getFeatures(cv2.cvtColor(frames[i],cv2.COLOR_RGB2GRAY),bboxs[i])
            # Only update if we got valid features
            if np.sum(new_startXs!=-1) > 0:
                startXs = new_startXs
                startYs = new_startYs
            else:
                print('Warning: Could not generate new features, keeping old ones')
